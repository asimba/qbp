...
inline uint32_t rc32_putc(uint32_t c,FILE *ofile){
  cntx=cntxs[cmask];
  uint16_t fc=fcs[cntx];
  uint8_t t=c;
...
void pack_file(FILE *ifile,FILE *ofile){
  uint8_t symbol=0;
  for(;;){
    if((rbuf(&symbol,ifile),rpos)){
      if(rc32_putc(symbol,ofile)) return;
    }
    else{
      if(rc32_putc(256,ofile)) return;
      for(int i=4;i;i--){
