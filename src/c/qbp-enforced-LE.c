...
inline uint32_t rc32_putc(uint32_t c,FILE *ofile,const uint8_t cntx){
  ...
  while((low^(low+range))<0x1000000||range<fc){
    if(!(wbuf(low>>24,ofile),wpos)) return 1;
    rc32_shift();
  };
...
void pack_file(FILE *ifile,FILE *ofile){
  ...
  for(;;){
    ...
    *cbuffer<<=flags;
    ...
    if(!length){
      for(int i=4;i;i--){
        if(!(wbuf(low>>24,ofile),wpos)) return;
        low<<=8;
      };
...
void unpack_file(FILE *ifile, FILE *ofile){
  ...
  for(;;){
    ...
    if(flags){
      length=rle_flag=1;
      if(cflags&0x80){
        if(rc32_getc(&c,ifile,vocbuf[(uint16_t)(vocroot-1)])) return;
        offset=c;
      }
      else{
        for(c=1;c<4;c++)
